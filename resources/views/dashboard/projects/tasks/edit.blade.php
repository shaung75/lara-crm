...
<p class="small">
    ...
    <a href="{{ route('task', $task->id) }}">{{ $task->title }}</a> |
    Edit
</p>
...
<div class="row">
    <div class="col-md-8">
        <div class="card">
            <div class="header">
                <h4 class="title">Edit "{{ $task->title }}"</h4>
            </div>
            <div class="content">
                <form method="POST" action="{{ route('task.update', [$task->project->id, $task->id]) }}">
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            ...
                            <div class="checkbox">
                                <input id="checkbox1" type="checkbox" name="completed" {{ $task->completed ? 'checked' : '' }}>
                    ...
                    <div class="row">
                        <div class="col-md-12">
                            <div class="form-group">
                                <label>Notes</label>
                                <textarea rows="5" class="form-control" name="notes" placeholder="Task notes">{{ $task->notes }}</textarea>
                            </div>
                        </div>
                    </div>

                    <button type="submit" class="btn btn-info btn-fill pull-right">Edit Task</button>
